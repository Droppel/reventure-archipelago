...
class BoTealLotusWorld(World):
    ...
    # web = web_world.BoTealLotusWebWorld()

    # This is how we associate the options defined in our options.py with our world.
    options_dataclass = options.BoTealLotusOptions
    options: options.BoTealLotusOptions  # Common mistake: This has to be a colon (:), not an equals sign (=).

    # Our world class must have a static location_name_to_id and item_name_to_id defined.
    # We define these in regions.py and items.py respectively, so we just set them here.
    location_name_to_id = locations.LOCATION_NAME_TO_ID
    item_name_to_id = {name: data.id for name, data in items.ITEM_NAME_TO_ID.items()}

    # There is always one region that the generator starts from & assumes you can always go back to.
    # This defaults to "Menu", but you can change it by overriding origin_region_name.
    origin_region_name = "Intro"
    ...
    def create_item(self, name: str) -> items.BoTealLotusItem:
