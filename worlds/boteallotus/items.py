...
class ItemData():
    def __init__(self, id: int, classification: ItemClassification, count: int = 1, event: bool = False, filler: bool = False):
        self.id = id
        self.count = count
...
ITEM_NAME_TO_ID = {
    ...
    "Kodama": ItemData(200, ItemClassification.progression),
}

# Each Item instance must correctly report the "game" it belongs to.
# To make this simple, it is common practice to subclass the basic Item class and override the "game" field.
class BoTealLotusItem(Item):
    game = "BoTealLotus"
...
def get_random_filler_item_name(world: BoTealLotusWorld) -> str:
    return "10 Fox Fire"


def create_item_with_correct_classification(world: BoTealLotusWorld, name: str) -> BoTealLotusItem:
    # Our world class must have a create_item() function that can create any of our items by name at any time.
    # So, we make this helper function that creates the item by name with the correct classification.
    # Note: This function's content could just be the contents of world.create_item in world.py directly,
    # but it seemed nicer to have it in its own function over here in items.py.
    classification = ITEM_NAME_TO_ID[name].classification

    return BoTealLotusItem(name, classification, ITEM_NAME_TO_ID[name], world.player)


# With those two helper functions defined, let's now get to actually creating and submitting our itempool.
def create_all_items(world: BoTealLotusWorld) -> None:
    ...
    itempool: list[Item] = []
    for name, data in ITEM_NAME_TO_ID.items():
        if data.event or data.filler: # Skip Events, Special items and Filler items
            continue
        for _ in range(data.count):
            item = BoTealLotusItem(name,data.classification, data.id, world.player)
