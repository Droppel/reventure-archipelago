...
def set_rules(options: PerGameCommonOptions, multiworld: MultiWorld, p: int):
    ...
    set_rule(multiworld.get_location("77_GetIntoTheCloud", p), lambda state: state.has("GrowVine", p) or (state._reventure_has_chicken(p) and (state._reventure_has_sword(p)
             or state.has("SpawnHookChest", p) or (state._reventure_can_pass_castle_with_item and state.has("UnlockGeyserWaterfall", p)))))
    ...
    set_rule(multiworld.get_location("85_KillAllFairies", p), lambda state: state._reventure_has_sword(p) or state.has("SpawnMrHugsChest", p)
             or (state.has("SpawnBoomerang", p) and (state.has("SpawnHookChest", p) or state._reventure_has_chicken(p))))
    ...
    set_rule(multiworld.get_location("89_FindAlienLarvae", p), lambda state: (options.hardjumps == 1 or state.has("SpawnLavaTrinketChest", p)) and state.has_any(["SpawnShovelChest", "SpawnPrincessItem"], p))
    ...
    set_rule(multiworld.get_location("98_FeedTheMimic", p), lambda state: state._reventure_has_burger(p) and state.has("SpawnMimic", p)
             and (state.has_any(["SpawnHookChest", "UnlockElevatorButton", "UnlockCallElevatorButtons"], p) or state._reventure_can_reach_princessportal_with_item(p)))
