...
def set_rules(options: PerGameCommonOptions, multiworld: MultiWorld, p: int):
    ...
    set_rule(multiworld.get_location("74_ShotgunFakePrincess", p), lambda state: state._reventure_has_sword(p) and state.has_all(["SpawnShopkeeper", "SpawnMimic", "UnlockShopCannon"], p) and state.has_any(["UnlockCallElevatorButtons", "UnlockElevatorButton"], p))
    ...
    set_rule(multiworld.get_location("87_KillAllDevsHell", p), lambda state: state._reventure_has_sword(p) and state._reventure_can_reach_princess_with_item(p) and (state.has("SpawnHookChest", p) or (state._reventure_has_chicken(p) and state.has("SpawnShovelChest", p))))
