...
def set_rules(options: PerGameCommonOptions, multiworld: MultiWorld, p: int):
    ...
    if options.treasureSword:
        def has_sword(state: CollectionState, p):
            if state.has("ProgressiveSword", p, 2):
                ...
                return state.has("SpawnShovelChest", p) and state.has_any(["SpawnHookChest", "UnlockGeyserWaterfall"], p)
            else:
                return False
    else:
        def has_sword(state: CollectionState, p):
    ...
    if options.hardjumps:
        ...
        set_rule(multiworld.get_location("61_DontKillMinions", p), lambda state: state.has("SpawnHookChest", p)
             or (can_pass_castle_with_item(state, p) and has_chicken(state, p)))
    ...
    if options.hardjumps:
        ...
        set_rule(multiworld.get_location("68_DarkLordComicStash", p), lambda state: state.has("SpawnPrincessItem", p))
