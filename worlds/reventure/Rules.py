...
def set_rules(options: PerGameCommonOptions, multiworld: MultiWorld, p: int):
    ...
    set_rule(multiworld.get_location("DesertEnd", p), lambda state: state._reventure_has_weight(p, 4) and state.has_all(["SpawnHookChest", "UnlockGeyserDesert2"], p))
    ...
    set_rule(multiworld.get_location("MultipleDesertJumps", p), lambda state: state._reventure_has_weight(p, 4) and state.has("SpawnHookChest", p))
